          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildResponsiveSearchBar(isDesktop, searchHintSize, buttonTextSize),
                  ...
                  child: Obx(() => Table(
                        ...
                        children: [
                          ...
                          TableRow(
                            ...
                            children: [
                              _buildTableCell("Sr.", fontSize: tableHeaderSize, isHeader: true),
                              _buildTableCell("Engine Model", fontSize: tableHeaderSize, isHeader: true),
                              _buildTableCell("Type", fontSize: tableHeaderSize, isHeader: true),
                              _buildTableCell("Recipe ID", fontSize: tableHeaderSize, isHeader: true),
                              _buildTableCell("Actions", fontSize: tableHeaderSize, isHeader: true),
                            ],
                          ),
                          ...
                          ...controller.recipeList.map((item) => TableRow(
                                children: [
                                  _buildTableCell(item.sr, fontSize: tableCellSize),
                                  _buildTableCell(item.model, fontSize: tableCellSize),
                                  _buildTableCell(item.type, fontSize: tableCellSize),
                                  _buildTableCell(item.recipeId, fontSize: tableCellSize),
                                  TableCell(
                                    child: Row(
                                      mainAxisAlignment: MainAxisAlignment.center,
                                      children: [
                                        IconButton(
                                          icon: Icon(Icons.edit, color: Colors.blue, size: isDesktop ? 22 : 18),
                                          onPressed: () => Get.toNamed(Routes.recipeAdditionReadOnlyScreen, arguments: item),
                                        ),
                                        IconButton(
                                          icon: Icon(Icons.download, color: Colors.green, size: isDesktop ? 22 : 18),
                                          onPressed: () => controller.exportSingleRecipe(item),
                                        ),
                                        // IconButton(
...
Widget _buildTableCell(String text, {required double fontSize, bool isHeader = false}) {
  return Padding(
    padding: EdgeInsets.symmetric(vertical: isHeader ? 16 : 12, horizontal: 4),
    child: Text(
      text,
...
Widget _buildResponsiveSearchBar(bool isDesktop, double searchFontSize, double buttonFontSize) {
  if (isDesktop) {
    return Row(
      children: [
        ...
        const SizedBox(width: 15),
        _buildActionButton("Add", () => Get.toNamed(Routes.recipeAdditionScreen), buttonFontSize),
      ],
    );
  } else {
    return Column(
      children: [
        ...
        Row(
          children: [
            Expanded(child: _buildActionButton("Search", () {}, buttonFontSize)),
            const SizedBox(width: 15),
            Expanded(child: _buildActionButton("Add", () => Get.toNamed(Routes.recipeAdditionScreen), buttonFontSize)),
          ],
        ),
      ],
    );
