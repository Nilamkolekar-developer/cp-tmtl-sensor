...
class SensorAnalysisScreen extends StatelessWidget {
          ...
          child: Column(
            ...
            children: [
              ...
              _buildSectionHeader(
                ...
                trailing: Row(
                  children: [
                    // --- NEW OPTION: PEAK FROM RECIPE ---

                    // Obx(() => controller.isAddingSensor.value
                    //     ? const SizedBox.shrink()
                    //     : Padding(
                    //         padding: const EdgeInsets.only(right: 10),
                    //         child: OutlinedButton.icon(
                    //           onPressed: () {
                    //             // Trigger the popup instead of just toggling a variable
                    //             _showRecipePicker(context);
                    //           },
                    //           icon: const Icon(
                    //               Icons.receipt_long_outlined), // Added an icon
                    //           label: const Text("Peak from Recipe"),
                    //           style: OutlinedButton.styleFrom(
                    //             foregroundColor: const Color(0xFF0055BB),
                    //             side:
                    //                 const BorderSide(color: Color(0xFF0055BB)),
                    //           ),
                    //         ),
                    //       )),

                    // --- EXISTING OPTION: ADD NEW SENSOR ---
  ...
  //           child: Text("No sensors in inventory.",
  ...
  //                   style: TextStyle(fontWeight: FontWeight.bold))),
  //                    DataColumn(
  //               label: Text('sampaling Rate',
  //                   style: TextStyle(fontWeight: FontWeight.bold))),
  //           DataColumn(
  //               label: Text('Action',
  //                   style: TextStyle(fontWeight: FontWeight.bold))),
  //         ],
  //         rows: controller.addedSensors
  //             .map((s) => DataRow(cells: [
  ...
  //                         onPressed: () => controller.startAnalysis(s),
  //                       ),
  //                       IconButton(
  //                         tooltip: "Delete",
  //                         icon: const Icon(Icons.delete_outline,
  //                             color: Colors.red),
  //                         onPressed: () => controller.addedSensors.remove(s),
  //                       ),
  //                     ],
  //                   )),
  //                 ]))
...
//       _drawText(
//           canvas, Offset(5, y - 12), labelValue.toStringAsFixed(1), textStyle);
//     }
...
//     final path = Path();
//     double dx =
//         size.width / (sampledPoints.length > 1 ? sampledPoints.length - 1 : 1);
...
//     List<Offset> allPeakOffsets = [];

//     // 3. Calculate Path, Find Min/Max, and Detect All Peaks
//     for (int i = 0; i < sampledPoints.length; i++) {
//       double norm = ((sampledPoints[i] - min) / range).clamp(0.0, 1.0);
//       double dy = size.height - (norm * size.height);
//       double currentX = i * dx;
//       Offset currentOffset = Offset(currentX, dy);

//       if (i == 0)
//         path.moveTo(0, dy);
//       else
//         path.lineTo(currentX, dy);

//       // --- TRACK GLOBAL MIN/MAX ---
//       if (sampledPoints[i] > currentMaxVal) {
//         currentMaxVal = sampledPoints[i];
//         maxPointOffset = currentOffset;
//       }
//       if (sampledPoints[i] < currentMinVal) {
//         currentMinVal = sampledPoints[i];
//         minPointOffset = currentOffset;
//       }

//       // --- DETECT LOCAL PEAKS ---
//       // A peak is higher than the point before and after it
//       if (i > 0 && i < sampledPoints.length - 1) {
//         if (sampledPoints[i] > sampledPoints[i - 1] &&
//             sampledPoints[i] > sampledPoints[i + 1]) {
//           allPeakOffsets.add(currentOffset);
//         }
//       }
//     }

//     // 4. Draw Gradient Fill and Line
...
//         colors: [const Color(0xFF0055BB).withOpacity(0.2), Colors.transparent],
//       ).createShader(Rect.fromLTWH(0, 0, size.width, size.height));

//     canvas.drawPath(fillPath, fillPaint);
//     canvas.drawPath(path, linePaint);

//     // 5. Draw EVERY Peak Point (Dots only, no numbers)
//     final peakDotPaint = Paint()
//       ..color = Colors.redAccent
//       ..style = PaintingStyle.fill;
//     for (var offset in allPeakOffsets) {
//       canvas.drawCircle(offset, 4,
//           Paint()..color = Colors.redAccent.withOpacity(0.2)); // Glow
//       canvas.drawCircle(offset, 2.5, peakDotPaint); // Solid dot
//     }

//     // 6. Draw MAX & MIN Highlights (With Labels)
//     _drawHighlight(canvas, maxPointOffset,
//         "MAX: ${currentMaxVal.toStringAsFixed(1)}", Colors.redAccent);
//     _drawHighlight(canvas, minPointOffset,
//         "MIN: ${currentMinVal.toStringAsFixed(1)}", Colors.green);
//   }

//   // Helper for labeled highlights (used for MIN and MAX)
...
//     _drawText(canvas, Offset(offset.dx - 20, offset.dy - 20), label,
...
//   @override
//   bool shouldRepaint(covariant IndustrialPainter oldDelegate) => true;
// }
class IndustrialPainter extends CustomPainter {
  final List<double> points;
  final double min, max;
